...
describe("AgentPassportV3 (AIFP-3 global identity)", function () {
  ...
  // Dimitry's review, item 2: the attestor must be able to be a Safe, so that
  // mirroring state on nine chains does not rest on one hot EOA key.
  describe("attestor may be a contract wallet (ERC-1271 / Safe)", function () {
    it("accepts attestations validated by a contract wallet", async function () {
      const ctx = await loadFixture(deploy);
      const safe = await (await ethers.getContractFactory("MockERC1271Wallet"))
        .deploy(await ctx.attestor.getAddress());
      await ctx.passport.connect(ctx.owner).setAttestor(await safe.getAddress());

      // Same EIP-712 signature, now validated through the wallet rather than
      // recovered to an EOA.
      const msg = {
        agentId: ctx.agentId, agentNumber: 123n, status: Status.ACTIVE,
    ...
    it("rejects a signature the contract wallet does not accept", async function () {
      const ctx = await loadFixture(deploy);
      const safe = await (await ethers.getContractFactory("MockERC1271Wallet"))
        .deploy(await ctx.attestor.getAddress());
      await ctx.passport.connect(ctx.owner).setAttestor(await safe.getAddress());
    ...
    it("can rotate from an EOA attestor to a contract wallet and back", async function () {
      const ctx = await loadFixture(deploy);
      const safe = await (await ethers.getContractFactory("MockERC1271Wallet"))
        .deploy(await ctx.attestor.getAddress());

      await register(ctx);                                   // EOA attestor
      await ctx.passport.connect(ctx.owner).setAttestor(await safe.getAddress());
      await bind(ctx, ctx.walletA, Binding.ACTIVE, 2n);      // same signer, via wallet
      expect(await ctx.passport.isAuthorizedWallet(await ctx.walletA.getAddress())).to.equal(true);

      await ctx.passport.connect(ctx.owner).setAttestor(await ctx.attestor.getAddress());
      await bind(ctx, ctx.walletB, Binding.ACTIVE, 3n);      // back to the EOA
      expect(await ctx.passport.isAuthorizedWallet(await ctx.walletB.getAddress())).to.equal(true);
    });
  });
