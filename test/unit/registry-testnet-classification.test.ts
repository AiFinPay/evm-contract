...
// ESM scripts are imported once at module load; no network context is needed.
const mod = await import("../../scripts/lib/testnet.mjs");
const classifyNetwork = mod.classifyNetwork;
const TESTNET_CHAIN_IDS = mod.TESTNET_CHAIN_IDS;

describe("registry: what counts as a testnet", () => {
  it("accepts a mainnet route that does not claim to be a testnet", () => {
    const r = classifyNetwork({ chainId: 137 });
    expect(r.ok).to.equal(true);
    expect(r.testnet).to.equal(false);
  });

  it("REFUSES testnet: true on a mainnet chain", () => {
  ...
  it("REFUSES a testnet chain that does not declare itself", () => {
  ...
  it("accepts a declared testnet on a known testnet chain", () => {
    const r = classifyNetwork({ chainId: 80002, testnet: true });
    expect(r.ok).to.equal(true);
    expect(r.testnet).to.equal(true);
  });

  it("keeps the testnet set closed and small", () => {
  ...
  it("treats a string chain id the same as a number", () => {
