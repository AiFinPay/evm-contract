...
describe("B2BSplitter v1.4 — native settlement", () => {
  ...
  it("moves exactly gross and leaves no value in the splitter", async () => {
    ...
    const [merchantAmt, treasuryAmt] = await splitter.quoteTotal(
      gross,
      routeIdMerchant,
      ethers.ZeroAddress,
    );
  ...
  it("rejects an expired signature", async () => {
    ...
    await expect(
      settleNative(fixture, { grossAmount: gross, validUntil: expired }),
    ).to.be.revertedWithCustomError(splitter, "SignatureExpired");
  });
  ...
  it("rejects replay of the same nonce", async () => {
    ...
    const gross = 10_000n;
    await settleNative(fixture, {
      grossAmount: gross,
      orderIdHash: ethers.keccak256(ethers.toUtf8Bytes("first")),
    });
  ...
  it("rejects an unknown route", async () => {
    ...
    await expect(
      settleNative(fixture, { grossAmount: gross, routeId: unknownRoute }),
    ).to.be.revertedWithCustomError(profiles, "UnknownRoute");
  });
  ...
  it("rejects a disabled route", async () => {
    ...
    ).to.be.revertedWithCustomError(splitter, "RouteDisabled");
  });
  ...
  it("rejects stable token on native path", async () => {
    ...
    const gross = 10_000n;
    await expect(
      settleNative(fixture, {
        grossAmount: gross,
        token: await usdc.getAddress(),
        routeId: routeIdAgent,
      }),
...
describe("B2BSplitter v1.4 — stable settlement", () => {
  it("splits USDC gross 99/1/0 without adding a fee on top", async () => {
    ...
    expect(await usdc.balanceOf(await merchant.getAddress())).to.equal(
      (gross * 9_900n) / BPS_DENOMINATOR,
    );
    expect(await usdc.balanceOf(await treasury.getAddress())).to.equal(
      (gross * 100n) / BPS_DENOMINATOR,
    );
  });
  ...
  it("rejects unsupported token", async () => {
    ...
    const quote = await buildQuote(fixture, {
      token: await rogue.getAddress(),
      grossAmount: ONE_USDC,
    });
    const signature = await signQuote(fixture.signer, splitter, quote);
    await expect(
      splitter.connect(agent).settleStable(quote, signature),
