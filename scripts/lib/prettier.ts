...
const PRETTIER_GLOB = "contracts/**/*.sol";
...
/**
 * Run Prettier on the Solidity contracts with the configured plugin.
 *
 * Exits the process with Prettier's status code, or 1 if no real Node binary
 * can be found.
 */
export function runPrettier(mode: PrettierMode): never {
  ...
  const result = spawnSync(
    nodeBin,
    [PRETTIER_BIN, flag, `--plugin=${PRETTIER_PLUGIN}`, PRETTIER_GLOB],
    {
      stdio: "inherit",
